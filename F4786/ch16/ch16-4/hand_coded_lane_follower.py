...
class HandCodedLaneFollower(object):
    ...
    def steer(self, frame, lane_lines):
        ...
        if self.car is not None:
            self.car.motor.turn_angle(angle=self.curr_steering_angle)
...
def detect_lane(frame):
    ...
    if _CROP_IMAGE: 
        ...
        x = 0 
        y = int((height * 1 / 2) + _HEIGHT_OFFSET)
        w = width
        h = int((height * 1 / 2) + _HEIGHT_OFFSET)
...
def detect_edges(frame):
    if _LANE_COLOR == "white":
        # filter for blue lane lines 
        mask = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        show_image("White Mask", mask)
    if _LANE_COLOR == "black":
...
def region_of_interest(canny):
    ...
    polygon = np.array([[
        (0, (height * 1 / 2) + _HEIGHT_OFFSET),
        (width, (height * 1 / 2) + _HEIGHT_OFFSET),
        (width, height),
        (0, height),
    ]], np.int32)
...
def compute_steering_angle(frame, lane_lines):
    ...
    if len(lane_lines) == 1:
        ...
        camera_mid_offset_percent = _MID_OFFSET_PERCENT # 0.0 means car pointing to center, -0.03: car is centered to left, +0.03 means car pointing to right
...
def display_heading_line(frame, steering_angle, line_color=(0, 0, 255), line_width=5, ):
    ...
    x2 = int(x1 - (height / 2 + _HEIGHT_OFFSET) / math.tan(steering_angle_radian))

    if _CROP_IMAGE:
        y2 = 0
    else:
        y2 = int(height / 2 + _HEIGHT_OFFSET)
...
def make_points(frame, line):
    ...
    if _CROP_IMAGE:
        ...
        y2 = int(y1 * 1 / 2 + _HEIGHT_OFFSET)  # make points from middle of the frame down
